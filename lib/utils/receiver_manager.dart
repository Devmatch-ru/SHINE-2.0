...
class ReceiverManager {
  ...
  Future<void> startDiscoveryListener() async {
    try {
      ...
      if (wifiIP == null) {
        ...
        throw Exception('Could not determine optimal Wi-Fi IP');
      }

      _addMessage('Using optimal WiFi IP for discovery: $wifiIP');

      _udpSocket = await RawDatagramSocket.bind(InternetAddress.anyIPv4, AppConstants.discoveryPort,
          reuseAddress: true);
      final response = '${AppConstants.receiverPrefix}$wifiIP:${AppConstants.signalingPort}';

      _udpSocket!.listen((event) {
        if (event == RawSocketEvent.read) {
          final datagram = _udpSocket!.receive();
          if (datagram != null &&
              String.fromCharCodes(datagram.data) == AppConstants.discoveryMessage) {
            _udpSocket!.send(
              response.codeUnits,
              datagram.address,
              datagram.port,
      ...
      await _startSignalServer();
      _addMessage('Discovery listener started on port ${AppConstants.discoveryPort} with IP $wifiIP');
    } catch (e) {
  ...
  Future<void> _startSignalServer() async {
    try {
        ...
        if (request.method == 'POST' && request.url.path == 'offer') {
          try {
            ...
            final pc = await createPeerConnection({
              'iceServers': AppConstants.iceServers, // ИСПРАВЛЕНИЕ: Используем константы
      ...
      _server = await shelf_io.serve(handler, InternetAddress.anyIPv4, AppConstants.signalingPort,
          shared: true);
      _addMessage('Signal server started on port ${AppConstants.signalingPort}');
    } catch (e) {
  ...
  Future<RTCDataChannel?> _handleDataChannelReconnect(
    ...
    try {
      ...
      if (pc != null) {
        ...
        int attempts = 0;
        const maxAttempts = AppConstants.maxRetries; // ИСПРАВЛЕНИЕ: Используем константу
        ...
        while (attempts < maxAttempts &&
          ...
          try {
            newChannel = await pc.createDataChannel(
              ...
              RTCDataChannelInit()
                ..ordered = true
                ..maxRetransmits = AppConstants.maxRetries // ИСПРАВЛЕНИЕ: Используем константу
                ..protocol = 'sctp'
                ..negotiated = false,
            );
  ...
  // ИСПРАВЛЕНИЕ 1: Улучшенная отправка команд с лучшей обработкой ошибок
  Future<void> sendCommand(String command) async {
    _addMessage('Attempting to send command: $command');
    ...
    // Ждем открытия канала данных с увеличенным таймаутом
    int attempts = 0;
    const maxWaitAttempts = 150; // 15 секунд
    ...
    try {
      ...
      // Попытка пересоздать канал данных
      final newChannel = await _handleDataChannelReconnect(_primaryBroadcaster!);
      if (newChannel == null) {
      ...
      try {
        await newChannel.send(RTCDataChannelMessage(commandData));
        _addMessage('Command sent successfully after channel recreation');
      } catch (retryError) {
        throw Exception('Failed to send command after retry: $retryError');
      }
    }
  }

  // ИСПРАВЛЕНИЕ 2: Улучшенная смена качества с правильной обработкой
  Future<void> changeStreamQuality(dynamic quality) async {
    _addMessage('Attempting to change stream quality to: $quality');
    ...
    // Убеждаемся, что канал данных открыт
    int attempts = 0;
    while (_dataChannels[_primaryBroadcaster]!.state !=
        RTCDataChannelState.RTCDataChannelOpen &&
        attempts < 100) {
    ...
    switch (quality.toString()) {
      ...
      case 'ReceiverStreamQuality.high':
        ...
        break;
      default:
        qualityString = quality.toString().toLowerCase();
        // Если это уже строка, используем её
        if (!['low', 'medium', 'high'].contains(qualityString)) {
          qualityString = 'medium';
        }
    }
